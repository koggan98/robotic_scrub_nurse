...
- `tray_camera_volume` publishes a `[0.14, 0.04, 0.04] m` frame box in `tray_camera_color_optical_frame` and a world-frame camera stand. The stand uses a `[0.05, 0.05, 0.52] m` vertical post with its top aligned to the camera height, plus bottom-mounted horizontal boxes that follow the post bottom.
